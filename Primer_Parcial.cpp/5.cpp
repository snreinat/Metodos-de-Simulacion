...
public:
  ...
  double GetDistance(void){return r.norm();};
...
int main(void){
  ...
  Colisionador Hertz;
  Crandom ran64(1);
  double m0=1.0, R0=3.0, kT=10.0, V0=1, Q=1;
  int i,ix,iy;
 ...
 //-------------------------(       x0,   y0,           Vx0,           Vy0, m0, R0)
  for(i=0; i<N; i++){
    ...
    Particula[i].Inicie(40*cos(i*2*M_PI/N), 40*sin(i*2*M_PI/N), V0*cos(Theta),V0*sin(Theta), m0, R0); // La partícula debe estar a una distancia de Ranillo del centro, en el eje x
  }
   ...
   for(t=0; t<tmax; t+=dt){
    for(i=0;i<N;i++) {
      ...
      cout<<t<<"\t"<<Particula[i].GetDistance()<<endl;
    }
