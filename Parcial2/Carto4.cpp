...
const int Lx=512; // #celdas en X
const int Ly=64; // #celdas en Y
...
double LatticeBoltzmann::C(int ix, int iy){
  ...
  double ix_right=ix_left+W*sqrt((1-(2*iy/Ly)-1)*((2*iy/Ly)-1));
  
  double ns=(nl-1.0)*(tanh(ix-ix_left)-tanh(ix-ix_right))/2;//n de la lente que suaviza
  double C0=0.5/ns;
  return C0;
}
...
double  LatticeBoltzmann::feq(double rho0,double Jx0,double Jy0,int i,int ix, int iy){
  if(i>0)
    return 3*w[i]*(pow(C(ix,iy),2)*rho0+Vx[i]*Jx0+Vy[i]*Jy0);
  else
    return rho0*(1-3*pow(C(ix,iy),2)*(1-W0));
}  
...
void LatticeBoltzmann::Print(const char * NameFile){
  ...
  for(ix=0;ix<Lx/2;ix++){ // imprimir rango ix = [0,200]
