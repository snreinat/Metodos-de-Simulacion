...
void LatticeBoltzmann::ImposeFields(int t){
  ...
  //Fuente plana a lo largo de todos los puntos ix=0
  ix=0; 
  for(iy=0;iy<Ly;iy++){
...
void LatticeBoltzmann::Print(const char * NameFile){
  ...
  for(ix=0;ix<Lx/2;ix++){ // imprimir rango ix = [0,Lx/2]
    for(iy=0;iy<Ly;iy++){ // imprimir rango iy== [0,Ly]
      rho0=rho(ix,iy,true);
